/**
*   Classe pai para todos os Models
*
* ModelConstructor é uma classe que encapsulam toda a lógica de definição e criação
...
var pModelConstructor = ctor(function(proto, _, _protected, __, __private){
    ...
    proto.define = function(db){
        ...
        var self = this;
        var defined = db.define(self_protected.name, self_protected.schema, {
        ...
        console.log(colors.yellow(defined.findAll));
        // Assimila o objeto Model do sequelize ao prototipo publico desta classe
        this.findOne = defined.findOne;
        this.findAll = defined.findAll;
    ...
    _protected.extend = function (src){
        for (var key in src) {
            if (src.hasOwnProperty(key)) this[key] = src[key];
        }
    }
    ...
    */
    proto.associate = function(associationObj, models){
        if(!_(this).isCommitted){
            throw new Error.InvalidOperationError('can\'t associate without define the model.');
        }
    ...
    proto.setInstanceMethods = function(instanceMethods){
